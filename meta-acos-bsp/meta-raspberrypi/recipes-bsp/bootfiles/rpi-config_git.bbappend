...
do_deploy:append() {
    ...
    echo "dtparam=audio=on" >> ${DEPLOYDIR}/bootfiles/config.txt
        echo "# Enable seeed 2-Channel CAN-BUS(FD) Shield (MCP2517FD)" >> ${DEPLOYDIR}/bootfiles/config.txt       
        echo "dtoverlay=seeed-can-fd-hat-v1" >> ${DEPLOYDIR}/bootfiles/config.txt
}
...
do_deploy:append() {
    ...
    echo "# Enable seeed 2-Channel CAN-BUS(FD) Shield (MCP2517FD)" >> $CONFIG
